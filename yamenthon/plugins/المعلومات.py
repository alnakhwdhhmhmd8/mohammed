...
async def _(event):
    ...
    OUTPUT = f"**⌔∮ [𝗦𝗢𝗨𝗥𝗖𝗘 𝐉𝐀𝐊-𝐀𝐓𝐇𝐎𝐍](tg://need_update_for_some_feature/) الاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝗦𝗢𝗨𝗥𝗖𝗘 𝐉𝐀𝐊-𝐀𝐓𝐇𝐎𝐍](tg://need_update_for_some_feature/) فـارات تـنصيـبك:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[𝗦𝗢𝗨𝗥𝗖𝗘 𝐉𝐀𝐊-𝐀𝐓𝐇𝐎𝐍](tg://need_update_for_some_feature/) , تم حساب سرعة السيرفر:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def zeddd(event):
    ...
    zedt = "**- تاريخ تنصيبـك لـ بـوت جـاكثون - 𓆩𝙎𝙊𝙐𝙍𝘾𝞝 𝐉𝐀𝐊-𝐀𝐓𝐇𝐎𝐍𓆪**\n\n"
